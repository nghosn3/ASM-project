...
%% notes
% need to exclude the alcohol from the total dose - go back and do that in
% plot_AED_connectivity script

% need to go back and make total dose based on periods of med admin, not on
% the days to have a little more granularity

%% for using polyfit to get smooth dose curve

%  %             time = 1:length(dose_per_day);
%             p = polyfit(time,dose_per_day,floor(x(end)./4)); %order determined by number of days that drug is given; d/dx=0 at max that many points
%             dose_curve = polyval(p,time);

%             figure;
%             subplot(2,1,1)
%             plot(time,dose_per_day);title([med_names{n} ' dose at each drug admin']) % need to include days where no drug is given;
%             subplot(2,1,2)
%             plot(time,dose_curve); title('poly-fit of dose curve')