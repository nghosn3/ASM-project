...
%%
diff_ativan = all_sz_data.closest_ativan-all_sz_data.seizure_time_emu % in hours
sz_ativan = (diff_ativan <1);

% get the clinical seizure types for ativan and not
conv_inds = contains(all_sz_data.SeizureType,'FBTCS');
all_sz_data.ativan_sz = sz_ativan;
all_sz_data.conv = conv_inds;

nonconv_inds = ~conv_inds;


[conttbl,chi2,p_chi,labels] = crosstab(all_sz_data.ativan_sz(:),all_sz_data.conv(:))
heatmap(all_sz_data,'ativan_sz','conv');
set(gca,'FontSize',16);
% p=0.25, fail to reject that clustering and ativan sz are independent (they likely are independent)

[h,p,stats] = fishertest(conttbl);

% 
% cd('/Users/ninaghosn/Documents/Litt_Lab/projects/Pioneer/AED-taper-networks/aed_dose_modeling')
